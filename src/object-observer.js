      ...
      value: function(observer, options) {
        ...
        if (options) {
          ...
          if ('pathsFrom' in options && options.path) {
            throw new Error(
              '"pathsFrom" option MAY NOT be specified together with "path" option'
            )
          }
          if (
            'pathsFrom' in options &&
            (typeof options.pathsFrom !== 'string' || !options.pathsFrom)
          ) {
            throw new Error(
              '"pathsFrom" option, if/when provided, MUST be a non-empty string'
            )
          }
          const invalidOption = Object.keys(options).find(
            option => !validOptionsKeys.includes(option)
          )
          if (invalidOption) {
            throw new Error(
              '"' +
                invalidOption +
                '" is not a one of the valid options (' +
                validOptionsKeys.join(', ') +
                ')'
            )
          }
        }
    ...
    while (l) {
      ...
      if (item && typeof item === 'object' && isObservableType(item)) {
        target[l] = Array.isArray(item)
          ...
          : new ObjectObserver({target: item, ownKey: l, parent: observer})
              .proxy
      } else {
        target[l] = item
      }
    ...
    while (l) {
      ...
      if (item && typeof item === 'object' && isObservableType(item)) {
        target[key] = Array.isArray(item)
          ? new ArrayObserver({target: item, ownKey: key, parent: observer})
              .proxy
          : new ObjectObserver({target: item, ownKey: key, parent: observer})
              .proxy
      } else {
        target[key] = item
      }
  ...
  callObservers = function(observers, changes) {
    ...
    for (target of observers.keys()) {
      try {
        ...
        if (options.path) {
          oPath = options.path
          relevantChanges = changes.filter(
            change => change.path.join('.') === oPath
          )
        } else if (options.pathsFrom) {
          oPaths = options.pathsFrom
          relevantChanges = changes.filter(change =>
            change.path.join('.').startsWith(oPaths)
          )
        }
...
class ObserverBase {
  constructor(properties, cloningFunction) {
    const source = properties.target,
  ...
  deleteProperty(target, key) {
    ...
    if (ad.observers.size) {
      ad.path.push(key)
      changes = [
        {type: DELETE, path: ad.path, oldValue: oldValue, object: this.proxy}
      ]
      callObservers(ad.observers, changes)
    }
  ...
  get(target, key) {
    const proxiedArrayMethods = {
      ...
      unshift: function proxiedUnshift(target, observed) {
        ...
        const unshiftResult = Reflect.apply(
          target.unshift,
          target,
          unshiftContent
        )
        ...
        for (let i = start, item, tmpTarget; i < end; i++) {
          ...
          if (item && typeof item === 'object' && isObservableType(item)) {
            target[i] = Array.isArray(item)
              ? new ArrayObserver({target: item, ownKey: i, parent: observed})
                  .proxy
              : new ObjectObserver({target: item, ownKey: i, parent: observed})
                  .proxy
          }
...
class Observable {
  constructor() {
    throw new Error(
      'Observable MAY NOT be created via constructor, see "Observable.from" API'
    )
  }
  ...
  static from(target) {
    if (
      ...
      if (!target || typeof target !== 'object') {
        throw new Error(
          'observable MAY ONLY be created from non-null object only'
        )
      } else if (
        'observe' in target ||
        'unobserve' in target ||
        ...
        throw new Error(
          target +
            ' found to be one of non-observable object types: ' +
            nonObservableTypes
        )
      }
    }
